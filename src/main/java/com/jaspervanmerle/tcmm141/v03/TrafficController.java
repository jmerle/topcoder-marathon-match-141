package com.jaspervanmerle.tcmm141.v03;
...
public class TrafficController {
    ...
    boolean isTurningHorizontal = true;

    public Set<Cell> getLightsToSwitch() {
        boolean done = true;

        for (Cell light : lights) {
            if ((light.isHorizontal && !isTurningHorizontal) || (!light.isHorizontal && isTurningHorizontal)) {
                done = false;
                break;
            }
        }
        ...
        if (done) {
            isTurningHorizontal = !isTurningHorizontal;
        }
        ...
        for (Road road : roads) {
            if ((road.isHorizontal && isTurningHorizontal) || (!road.isHorizontal && !isTurningHorizontal)) {
                continue;
            }
            ...
            for (Cell cell : road.cells) {
                lightSeen = lightSeen || cell.hasLight;
                carSeen = carSeen || (lightSeen && cell.car != null && cell.car.road == road);

                if (lightSeen && carSeen && cell.hasLight) {
